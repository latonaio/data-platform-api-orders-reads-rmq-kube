...
func (c *DPFMAPICaller) Item(
	...
	rows, err := c.db.Query(
		`SELECT OrderID, OrderItem, OrderItemCategory,
    		SupplyChainRelationshipID, SupplyChainRelationshipDeliveryID, SupplyChainRelationshipDeliveryPlantID,
			SupplyChainRelationshipStockConfPlantID, SupplyChainRelationshipProductionPlantID, OrderItemText,
			OrderItemTextByBuyer, OrderItemTextBySeller, Product,
			ProductStandardID, ProductGroup, BaseUnit,
			PricingDate, PriceDetnExchangeRate, RequestedDeliveryDate,
			DeliverToParty, DeliverFromParty, DeliverToPlant,
			DeliverToPlantTimeZone, DeliverToPlantStorageLocation, ProductIsBatchManagedInDeliverToPlant,
			BatchMgmtPolicyInDeliverToPlant, DeliverToPlantBatch, DeliverToPlantBatchValidityStartDate,
			DeliverToPlantBatchValidityEndDate, DeliverFromPlant, DeliverFromPlantTimeZone,
			DeliverFromPlantStorageLocation, ProductIsBatchManagedInDeliverFromPlant, BatchMgmtPolicyInDeliverFromPlant,
			DeliverFromPlantBatch, DeliverFromPlantBatchValidityStartDate, DeliverFromPlantBatchValidityEndDate,
			DeliveryUnit, StockConfirmationBusinessPartner, StockConfirmationPlant,
			StockConfirmationPlantTimeZone, StockConfirmationPlantBatch, StockConfirmationPlantBatchValidityStartDate,
			StockConfirmationPlantBatchValidityEndDate, ProductIsBatchManagedInStockConfirmationPlant, BatchMgmtPolicyStockConfirmationInPlant,
			ServicesRenderingDate, OrderQuantityInBaseUnit, OrderQuantityInDeliveryUnit,
			StockConfirmationPolicy, StockConfirmationStatus, ConfirmedOrderQuantityInBaseUnit,
			ItemWeightUnit, ProductGrossWeight, ItemGrossWeight,
			ProductNetWeight, ItemNetWeight, NetAmount, TaxAmount, GrossAmount,
			InvoiceDocumentDate, ProductionPlantBusinessPartner, ProductionPlant,
			ProductionPlantTimeZone, ProductionPlantStorageLocation, ProductIsBatchManagedInProductionPlant,
			BatchMgmtPolicyInProductionPlant, ProductionPlantBatch, ProductionPlantBatchValidityStartDate,
			ProductionPlantBatchValidityEndDate, Incoterms, TransactionTaxClassification,
			ProductTaxClassificationBillToCountry, ProductTaxClassificationBillFromCountry, DefinedTaxClassification,
			AccountAssignmentGroup, ProductAccountAssignmentGroup, PaymentTerms,
			DueCalculationBaseDate, PaymentDueDate, NetPaymentDays,
			PaymentMethod, Project, AccountingExchangeRate,
			ReferenceDocument, ReferenceDocumentItem, ItemCompleteDeliveryIsDefined,
			ItemDeliveryStatus, IssuingStatus, ReceivingStatus,
			ItemBillingStatus, TaxCode, TaxRate,
			CountryOfOrigin, CountryOfOriginLanguage, ItemBlockStatus,
			ItemDeliveryBlockStatus, ItemBillingBlockStatus
