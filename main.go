...
import (
	dpfm_api_caller "data-platform-api-orders-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-orders-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncOrdersReads(accepter, &input, &output, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"Header", "HeaderPartner", "HeaderPartnerContact", "HeaderPartnerPlant",
			"Address", "Item", "ItemPartner", "ItemPartnerPlant",
			"ItemPricingElement", "ItemSchedulingLine",
		}
	}
	return accepter
}
